...
${(_d = result.error) != null ? _d : ""}`, 8e3);
  ...
  async syncAll() {
    ...
    for (const file of this.app.vault.getMarkdownFiles()) {
      ...
      if (localNewer) {
        const zone = (_e = fm["blog-zone"]) != null ? _e : blogPost.zone;
        const ok = await this.doPush(file, fm, blogId, zone);
        if (ok) pushed++;
      } else if (blogNewer) {
